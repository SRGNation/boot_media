	...
	<body>
      <?php PrintNavBar('home'); ?>
		<div class="container">
			...
			<p>Why not join the fun? <a class="btn btn-primary" href="signup.php">Create an Account</a> or <a class="btn btn-primary" href="signin.php">Login</a> if you already have one.</p> <?php } else {?>
				...
				<div class="panel-body">
					<?php if($rc_count == 0) {
						...
						while($rec_com = mysqli_fetch_array($get_rec_commun)) {
							echo '<list class="list-group-item"><img src="'.htmlspecialchars($rec_com['community_icon']).'" style="width:50px;height:50px;" class="img-rounded"> <a href="/communities.php?id='.$rec_com['id'].'">'.htmlspecialchars($rec_com['community_name']).'</a></list>';
						}
						echo '<list class="list-group-item"><a href="/list_communities.php?type=recommended">View More</a></list>';
					} ?>
				</div>
			</div>
		</div>
